        ...
        *,
        ...
        canonical_querystring = urllib.parse.urlencode(
            collections.OrderedDict(
                sorted(parameters.items(), key=lambda item: item[0])
            )
        )

        formatted_headers = {key.lower(): headers.get(key).strip() for key in headers}

        ordered_headers = collections.OrderedDict(
            sorted(formatted_headers.items(), key=lambda item: item[0])
        )

        canonical_headers = (
            "\n".join(f"{key}:{val}" for key, val in ordered_headers.items()) + "\n"
        )
